 ...
 * @author Asus
 */
public class AwaitBoilingWaterLocationPanel extends JPanel implements Observer {
 ...
 private ObservableGame game;
  ...
  public AwaitBoilingWaterLocationPanel(ObservableGame game )
    {
        this.game = game;
   ...
   int num  = game.getGameData().getCartaActua().getSerie();
        ...
        setVisible(game.getState() instanceof AwaitSelectionBoilingWaterAttack);
      
        if(game.getState() instanceof AwaitSelectionBoilingWaterAttack )
        {
       ...
       {
           escolhe1.setEnabled(true);
       }
       else
           escolhe1.setEnabled(false);
       
       if ((game.getGameData().getEnemy().isCirculoDoor()==true))
       {
           escolhe2.setEnabled(true);
       }
       else
           escolhe2.setEnabled(false);
       
       if ((game.getGameData().getEnemy().isCirculoTower()==true))
       {
           escolhe3.setEnabled(true);
       }
       else
           escolhe3.setEnabled(false);
...
//nao esquecer para baixo distancia de 38/ 27!!!!
   ...
   switch(game.getGameData().getEnemy().getPosicao("wall"))
 {
     ...
     case 0:  g.drawImage(SiegePanel.getPeca(),1008,747, SiegePanel.getPeca().getWidth()/3, SiegePanel.getPeca().getHeight()/3, this);
