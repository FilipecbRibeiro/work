 ...
 */
public class AwaitExtraSelectionPanel extends JPanel implements Observer{
  ...
  public AwaitExtraSelectionPanel(ObservableGame game )
    {
        this.game = game;
   ...
   actual = new JButton ( new ImageIcon((SiegePanel.getPeca().getScaledInstance(SiegePanel.getPeca().getWidth()/2,SiegePanel.getPeca().getHeight() ,Image.SCALE_SMOOTH))));
        actual.setBorder(BorderFactory.createEmptyBorder());
           ...
           escolhe1 = new JRadioButton ("Moral");
   ...
   int num  = game.getGameData().getCartaActua().getSerie();
        ...
        setVisible(game.getState() instanceof AwaitExtraSelection);
      
        if(game.getState() instanceof AwaitExtraSelection)
        {
